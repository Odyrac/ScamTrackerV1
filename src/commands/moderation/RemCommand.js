...
module.exports = class WarnCommand extends BaseCommand {
  constructor() {
    super('rem', 'moderation', [], [], '{pseudo} {raison}', 1, "Mets un membre dans le remboursé.", []);
  }
  ...
  async run(client, message, args, settings) {
    ...
    if (message.member.roles.cache.has('827624653442252881') || message.member.roles.cache.has('828015879421820989') || message.member.roles.cache.has('837644067578970143') || message.author.id === `423545393771577345`) {
      ...
      if (!pseudo) return badUsage(`${settings.prefix}rem {pseudo} {raison}`, message.channel);

      const embed = new MessageEmbed()
      .setTitle(`\`${pseudo2}\` a remboursé son stuff !`)
      .setThumbnail(`https://crafatar.com/renders/body/${uuid}?overlay`)
      .setColor("#ee942e")
      .setDescription(`Il n'est dorénavant plus présent comme scammer dans notre base de données. Néanmoins, il reste détectable avec la commande \`${settings.prefix}check ${pseudo2}\`.\n\n__**Ajouté par :**__ ${message.author}`)

      const logembed = new MessageEmbed()
      .setAuthor(`Un joueur a remboursé le stuff volé !`)
      .setColor('#ee942e')
      ...
      if (currentlyBL.length) {
        ...
        if (currentlyas.length) {
          return sendError(`Ce joueur a déjà été désigné comme ayant remboursé le stuff.`, message.channel)
        } else {
          return sendError(`Ce joueur n'est pas dans la ScamList, tu ne peux pas dire qu'il a remboursé.`, message.channel)
        }
      }
